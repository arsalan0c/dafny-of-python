...
let id_str : identifier -> string = function
...
let rec exp_z3 = function
  | Identifier(ident) -> if Hashtbl.mem env ident then Hashtbl.find env ident else 
    let ide = Arithmetic.Integer.mk_const_s ctx ident in
    let _ = Hashtbl.add env ident ide in
    ide
...
let stmt_z3 post = function
  | Assign(il, el) -> begin
    ...
    match e with
      | x -> 
        let id = id_str (List.nth il 0) in
...
let func_z3 = function
  | Function(Spec(pre, post), ident, il, sl) -> begin
    let f = (fun id -> let ids = (id_str id) in let ide = Arithmetic.Integer.mk_const_s ctx ids in Hashtbl.add env ids ide) in
      let _ = List.iter f il in
      match composition (exp_z3 pre) (exp_z3 post) (List.rev sl) with
        | Some x -> let name = (id_str ident) in 
