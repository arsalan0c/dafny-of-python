...
let rec stmt_for s = 
  ...
  | For (specl, el, e, sl) ->
    ...
    let assign_counter = Assign (Typ TVoid, [counter_ident], [Literal (IntLit 0)]) in
    let declare_target_vars = Assign (Typ TVoid, el, []) in
    ...
    let assign_targets = Assign (Typ TVoid, el, [domain_value]) in
    ...
    let update_counter = Assign (Typ TVoid, [counter_ident], [incr_counter]) in
